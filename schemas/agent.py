...
class Agent(BaseModel):
    ...
    role: str = Field(default="general_analysis", description="Configured role or archetype")
    status: str = Field(default="ACTIVE", description="Operational status: ACTIVE | IDLE | DEGRADED | BLOCKED | SWITCHING | FAILOVER")
    enabled: bool = Field(default=True, description="Whether agent is enabled by analyst")
    supported_models: List[str] = Field(default_factory=list, description="IDs of models supported by this agent")
    current_model_id: Optional[str] = Field(default=None, description="Currently active model ID")
