"""
LLMorch Adapters - Real Codex CLI Execution Boundary Implementation
Invokes installed Codex CLI via process isolation/subprocess supervision,
handles input formatting, streaming, process cancellation, and normalization into TaskResult.
"""
...
class CodexAdapter(BaseAgentAdapter):
    """
    Provider-neutral adapter implementation for real Codex CLI subprocess execution.
    """
    ...
    def _detect_cli_executable(self) -> str:
        """Detects available Codex CLI binary on the host system."""
        candidates = ["codex", "codex-cli", "openai-codex"]
    ...
    def start(self, task: Task) -> Run:
        """Starts real Codex process attempt for given Task."""
        run = Run(
    ...
    def execute_task_sync(self, task: Task, workspace_dir: str, context_prompt: str) -> Dict[str, Any]:
        """
        Launches real Codex process synchronously with process-group supervision.
    ...
    def normalize_result(self, raw_output: Any) -> TaskResult:
        """
        Parses raw text/JSON stdout from Codex into structured TaskResult contract.
        """
