"""
LLMorch Adapters - Real Claude Code CLI Execution Boundary Implementation
Invokes installed Claude Code CLI via process isolation/subprocess supervision,
...
class ClaudeAdapter(BaseAgentAdapter):
    ...
    def _detect_cli_executable(self) -> str:
        """Detects available Claude Code CLI binary on the host system."""
        candidates = ["claude", "claude-code", "claude-cli"]
    ...
    def start(self, task: Task) -> Run:
        """Starts real Claude Code process attempt for given Task."""
        run = Run(
    ...
    def normalize_result(self, raw_output: Any) -> TaskResult:
        """
        Parses raw text/JSON stdout from Claude Code into structured TaskResult contract.
        """
